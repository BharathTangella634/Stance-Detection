...
def compute_performance(preds,y,trainvaltest,step,args,seed):
    ...
    result_id = ['train', step, seed, args['dropout']]
...
def run_classifier():
    ...
    for seed in random_seeds:    
        ...
        log_dir = os.path.join('./tensorboard/tensorboard_train'+str(args['percent'])+'_d0'+str(args['dropout'])+'_d1'+str('_seed'+str(seed)), 'train')
        train_writer = SummaryWriter(log_dir=log_dir)

        log_dir = os.path.join('./tensorboard/tensorboard_train'+str(args['percent'])+'_d0'+str(args['dropout'])+'_d1'+str('_seed'+str(seed)), 'val')
        val_writer = SummaryWriter(log_dir=log_dir)

        log_dir = os.path.join('./tensorboard/tensorboard_train'+str(args['percent'])+'_d0'+str(args['dropout'])+'_d1'+str('_seed'+str(seed)), 'test')
...
if __name__ == "__main__":
    # You can use refineTrainData from utils to refine the train all onecol file.
    run_classifier()
